...
/*   Updated: 2025/12/16 23:25:56 by guillsan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline unsigned int	compute_color_julia_preview(int i, double magnitude_sq,
	t_fract *fract)
{
	const double	log_modulus = 0.5 * log(magnitude_sq);
	const double	nu = (double)i + 1.0 - (log(log_modulus) * LOG2);
	...
	if (i == STEPS)
		...
		color = get_clr_palette_threads(i, STEPS, nu, fract->clr_shift);
	return (color);
}

static inline unsigned int	compute_color_depth_julia_preview(int i, double magnitude_sq,
	t_fract *fract)
{
	(void)magnitude_sq;
	return (get_clr_palette_depth_preview(i, fract));
}
