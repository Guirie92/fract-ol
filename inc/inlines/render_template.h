...
/*   Updated: 2025/12/15 19:41:52 by guillsan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void	RENDER_BLOCK_JULIA(int bx, int by, t_fract *fract)
{
	int	s;
	int	px0;
	int	py0;
	...
	fract->clr = PALETTE(COMPUTE_PIX(px_center, py_center, fract), fract);
	fill_blocks_julia(py0, px0, s, fract);
}

static inline void	RENDER_LOOP_JULIA(t_fract *fract, int *tiles_x)
{
	int	rendered;
	int	shuf_idx;
	int	bx;
	...
	while (rendered < fract->ren.total_blocks)
	{
		fract->rcode = pass_check_julia(fract, tiles_x);
		if (fract->rcode == EBREAK)
			break ;
		else if (fract->rcode == ECONTINUE)
			continue ;
		...
		bx = shuf_idx % *tiles_x;
		by = shuf_idx / *tiles_x;
		RENDER_BLOCK_JULIA(bx, by, fract);
		fract->ren.pass_pix_idx++;
		fract->ren.comp_blocks++;
		rendered++;
		if (atomic_load(&fract->time.timeout_flag) == 1)
